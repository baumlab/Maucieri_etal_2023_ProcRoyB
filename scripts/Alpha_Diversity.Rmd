...
```{r Summary of Data, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r iNEXT, echo=FALSE, warning=FALSE, message=FALSE, fig.height=13, fig.width=12}
...
Figure_S2 <- ggiNEXT(out.raw, color.var ="site", se = FALSE) + theme_DGM() + theme(legend.position="bottom", legend.title=element_blank())
Figure_S2
...
```{r extrapolation plot, echo=FALSE, message=FALSE, warning=FALSE, fig.height=7.5, fig.width=12}
...
for (i in 1:nrow(extrapolation_df)) {
  ...
  xx <- est_i %>% filter(method == "extrapolated")
  extrapolated_number <- length(unique(xx$site))
...
```{r Coverage 85, echo=FALSE, warning=FALSE, fig.height=10, fig.width=16, message=FALSE}

est.raw <- estimateD(iNEXT_list, datatype="incidence_raw", base="coverage", level=0.85, conf=0.95) 
estimates_85 <- est.raw %>% tidyr::separate(site, c("Site", "Year"), "_KI", remove = TRUE)
...
```{r Coverage 90, echo=FALSE, warning=FALSE, fig.height=5, fig.width=16, message=FALSE}

est.raw <- estimateD(iNEXT_list, datatype="incidence_raw", base="coverage", level=0.90, conf=0.95) 
estimates_90 <- est.raw %>% tidyr::separate(site, c("Site", "Year"), "_KI", remove = TRUE)
...
```{r Coverage 90 Figure S5, echo=FALSE, warning=FALSE, fig.height=10, fig.width=16}
...
```{r Coverage 95, echo=FALSE, warning=FALSE, fig.height=10, fig.width=16, message=FALSE}

est.raw <- estimateD(iNEXT_list, datatype="incidence_raw", base="coverage", level=0.95, conf=0.95) 
estimates_95 <- est.raw %>% tidyr::separate(site, c("Site", "Year"), "_KI", remove = TRUE)
...
```{r models 85, echo=FALSE, warning=FALSE}
...
```{r models 90, echo=FALSE, warning=FALSE}
...
```{r models 95, echo=FALSE, warning=FALSE}
...
```{r Average losses total, echo=FALSE, warning=FALSE}
...
```{r Average losses HD cat, echo=FALSE, warning=FALSE}
...
```{r Effect Size Richness, echo=FALSE, warning=FALSE, fig.height=6, fig.width=4}

Xdi_raw_Richness <- estimates_90 %>% 
  filter(order == 0 & HD_Cat == "Very Low") %>% 
  group_by(Site) %>% 
...
for (i in 1:nrow(AR_Richness)) {
  ...
  Yci <- estimates_90 %>% filter(order == 0 & Site == AR_Richness$Site[i] & MHW == "Before") 
  Ymi <- estimates_90 %>% filter(order == 0 & Site == AR_Richness$Site[i] & MHW == "After") 
...
### (b) Shannon
```{r Effect Size Shannon, echo=FALSE, warning=FALSE, fig.height=6, fig.width=4}

Xdi_raw_Shannon <- estimates_90 %>% 
  filter(order == 1 & HD_Cat == "Very Low") %>% 
  group_by(Site) %>% 
  dplyr::select(Site, MHW, qD) %>% 
  pivot_wider(names_from = MHW, values_from = qD) %>% 
  mutate(Xd_numerator = (Before - After)/Before)
...
for (i in 1:nrow(AR_Shannon)) {
  ...
  Yci <- estimates_90 %>% filter(order == 1 & Site == AR_Shannon$Site[i] & MHW == "Before") 
  Ymi <- estimates_90 %>% filter(order == 1 & Site == AR_Shannon$Site[i] & MHW == "After") 
...
### (c) Simpson
```{r Effect Size 2 Simpson, echo=FALSE, warning=FALSE, fig.height=6, fig.width=4}

Xdi_raw_Simpson <- estimates_90 %>% 
  filter(order == 2 & HD_Cat == "Very Low") %>% 
  group_by(Site) %>% 
  dplyr::select(Site, MHW, qD) %>% 
  pivot_wider(names_from = MHW, values_from = qD) %>% 
  mutate(Xd_numerator = (Before - After)/Before)
...
for (i in 1:nrow(AR_Simpson)) {
  ...
  Yci <- estimates_90 %>% filter(order == 2 & Site == AR_Simpson$Site[i] & MHW == "Before") 
  Ymi <- estimates_90 %>% filter(order == 2 & Site == AR_Simpson$Site[i] & MHW == "After") 
